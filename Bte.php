...
class Bte
{
	...
	public function parse()
	{
		...
		$tpl = $this->parseBlock($this->tpl, $this->vars);
	...
	protected function parseBlock($subject,$vars)
	{
		$callback = new BlockReplacer($vars);

		// blocks
		$subject = preg_replace_callback($this->blockRegEx, array(&$this, "replaceBlockCallback"), $subject);
		// replace variables
		$subject = preg_replace_callback($this->varRegEx, array($callback, 'replaceVarCallback'), $subject);
		// replace arrays
...
class BlockReplacer {
    ...
    public function replaceVarCallback($matches)
		...
		protected function replaceBlockCallback($matches)
		{
			...
			if (!empty($this->hide[$blockName])) {
				return false;
			}
			if (empty($this->vars[$blockName])) {
				return false;
			}
